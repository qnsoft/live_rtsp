...
func (s *UDPServer) SetupAudio() (err error) {
	...
	if err := s.AConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp server audio conn set read buffer error, %v", err)
	}
	if err := s.AConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp server audio conn set write buffer error, %v", err)
	}
	...
	go func() {
		...
		for !s.Stoped {
			if n, _, err := s.AConn.ReadFromUDP(bufUDP); err == nil {
				...
				live_utils.Println("udp server read audio pack error", err)
				continue
			}
		}
	}()
	...
	go func() {
		...
		for !s.Stoped {
			if n, _, err := s.AControlConn.ReadFromUDP(bufUDP); err == nil {
				...
				live_utils.Println("udp server read audio control pack error", err)
				continue
			}
		}
	}()
...
func (s *UDPServer) SetupVideo() (err error) {
	...
	go func() {
		...
		for !s.Stoped {
			if n, _, err := s.VConn.ReadFromUDP(bufUDP); err == nil {
				...
				live_utils.Println("udp server read video pack error", err)
				continue
			}
		}
	}()
	...
	if err := s.VControlConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp server video control conn set read buffer error, %v", err)
	}
	if err := s.VControlConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp server video control conn set write buffer error, %v", err)
	}
	...
	go func() {
		bufUDP := make([]byte, UDP_BUF_SIZE)
		live_utils.Printf("udp server start listen video control port[%d]", s.VControlPort)
		defer live_utils.Printf("udp server stop listen video control port[%d]", s.VControlPort)
		for !s.Stoped {
			if n, _, err := s.VControlConn.ReadFromUDP(bufUDP); err == nil {
				...
				live_utils.Println("udp server read video control pack error", err)
				continue
			}
		}
	}()

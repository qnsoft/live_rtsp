		...
		if buf1 == 0x24 { //rtp data
			...
			case session.aRTPChannel:
				...
				if session.RtpAudio != nil {
					...
					if elapsed >= 30*time.Second {
						live_utils.Println("Recv an audio RTP package")
						timer = time.Now()
					}
			...
			case session.vRTPChannel:
				...
				if session.RtpVideo != nil {
					elapsed := time.Since(timer)
					if elapsed >= 30*time.Second {
						live_utils.Println("Recv an video RTP package")
						timer = time.Now()
					}
					...
					if len(line) == 0 {
						...
						if contentLen > 0 {
							...
							if n, err := io.ReadFull(session.connRW, bodyBuf); err != nil {
								...
								live_utils.Printf("read rtsp request body failed, expect size[%d], got size[%d]", contentLen, n)
								return
							}
...
func (session *RTSP) handleRequest(req *Request) {
	...
	defer func() {
		if p := recover(); p != nil {
			live_utils.Printf("handleRequest err ocurs:%v", p)
			res.StatusCode = 500
		...
		if res.StatusCode != 200 && res.StatusCode != 401 {
			live_utils.Printf("Response request error[%d]. stop session.", res.StatusCode)
			session.Stop()
		}
	}()
	session.URL = req.URL

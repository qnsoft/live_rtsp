...
type UDPClient struct {
	...
	AT           *live_sdk.AudioTrack
	APacketizer  rtp.Packetizer
	VT           *live_sdk.VideoTrack
...
func (c *UDPClient) SetupAudio() (err error) {
	...
	if err := c.AConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client audio conn set read buffer error, %v", err)
	}
	if err := c.AConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client audio conn set write buffer error, %v", err)
	}
	...
	if err := c.AControlConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client audio control conn set read buffer error, %v", err)
	}
	if err := c.AControlConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client audio control conn set write buffer error, %v", err)
	}
	return
}
...
func (c *UDPClient) SetupVideo() (err error) {
	...
	if err := c.VConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client video conn set read buffer error, %v", err)
	}
	if err := c.VConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client video conn set write buffer error, %v", err)
	}
	...
	if err := c.VControlConn.SetReadBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client video control conn set read buffer error, %v", err)
	}
	if err := c.VControlConn.SetWriteBuffer(networkBuffer); err != nil {
		live_utils.Printf("udp client video control conn set write buffer error, %v", err)
	}
	return
}

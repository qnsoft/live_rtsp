...
func runPlugin() {
	http.HandleFunc("/api/rtsp/list", func(w http.ResponseWriter, r *http.Request) {
		...
		for tick := time.NewTicker(time.Second); err == nil; <-tick.C {
			var info []*RTSP
			for _, s := range live_sdk.Streams.ToList() {
...
type RTSP struct {
	*live_sdk.Stream  `json:"-"`
	...
	HasVideo           bool
	HasAudio           bool
	RtpAudio           *live_sdk.RTPAudio
	RtpVideo           *live_sdk.RTPVideo
}
...
func (rtsp *RTSP) setAudioTrack() {
	...
	if len(rtsp.ASdp.Config) > 0 {
		...
		case "PCMU":
			...
			at.ExtraData = []byte{(at.CodecID << 4) | (1 << 1)}
		default:
			live_utils.Printf("rtsp audio codec not support:%s", rtsp.ASdp.Codec)
			return
		}
	}
	rtsp.RtpAudio = at

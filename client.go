		...
		case "video":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err = client.UDPServer.SetupVideo(); err != nil {
					live_utils.Printf("Setup video err.%v", err)
					return err
				}
		...
		case "audio":
			...
			if client.TransType == TRANS_TYPE_TCP {
				...
				if err = client.UDPServer.SetupAudio(); err != nil {
					live_utils.Printf("Setup audio err.%v", err)
					return err
				}
...
func (client *RTSP) startStream() {
	...
	defer func() {
		if client.Err() == nil && config.Reconnect {
			...
			live_utils.Printf("reconnecting:%s in 5 seconds", client.URL)
	...
	if err := client.requestStream(); err != nil {
		live_utils.Printf("rtsp requestStream err:%v", err)
		return
	}
	for client.Err() == nil {
		...
		if err != nil {
			live_utils.Printf("client.connRW.ReadByte err:%v", err)
			return
		}
		switch b {
		case 0x24: // rtp
			...
			if err != nil {
				live_utils.Printf("io.ReadFull err:%v", err)
				return
			}
			...
			default:
				live_utils.Printf("unknow rtp pack type, channel:%v", channel)
				continue
			}
		...
		default: // rtsp
			...
			for client.Err() == nil {
				...
				if err != nil {
					live_utils.Printf("client.connRW.ReadLine err:%v", err)
					return
				}
				if len(line) == 0 {
					if contentLen != 0 {
				...
				if strings.Index(s, "Content-Length:") == 0 {
					...
					if err != nil {
						live_utils.Printf("strconv.Atoi err:%v, str:%v", err, splits[1])
						return
					}
				}
			}
